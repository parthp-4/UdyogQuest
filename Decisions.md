...
## 2026-08-18 - Normalize Gemini's JSON response at the boundary, don't trust it as AssistantAnswer

- Decision: after `JSON.parse(text)` on Gemini's response in `lib/ai/gemini.ts`, run the result through a `normalizeAssistantAnswer()` function that reconstructs the exact `AssistantAnswer` shape field-by-field, rather than casting the parsed value directly (`as AssistantAnswer`). Applied the same normalize-before-use pattern client-side in `components/forms/assistant-panel.tsx` on the fetch response.
- Why: found live, from a real user-reported crash -- `missingInformation` (documented as `string[]`) came back from Gemini as a plain string, and the client's unconditional `.map()` call on it crashed the whole page with Next.js's client error boundary. `responseMimeType: "application/json"` guarantees syntactically valid JSON, not a specific field-level schema; a `JSON.parse(...) as T` cast is a compile-time-only assertion that does nothing at runtime, so any mismatch between what the model actually emits and the declared TypeScript type reaches the client unchanged. This is exactly a system/trust boundary (external, non-deterministic model output) where the project's own rule ("only validate at system boundaries") applies -- it was previously not validated at all.
- Alternatives considered: rely solely on a stricter system-prompt instruction (done as a supplementary fix, but rejected as the *only* fix -- prompt instructions reduce but do not guarantee a specific output shape from a generative model, and this exact deviation was already observed once despite a reasonable-looking prompt); use the Gemini SDK's `responseSchema` config for enforced structured output (not adopted in this pass -- a real improvement worth doing, but deferred as a separate, deliberate change rather than folding it into an urgent crash fix); only fix the client-side `.map` call (rejected as insufficient alone -- the server is the correct place to guarantee the contract once, rather than requiring every current and future consumer of `askGeminiFromSources()` to defensively re-validate it).
- Consequences: any future field Gemini might return in an unexpected shape is contained to whatever `normalizeAssistantAnswer()` explicitly reconstructs -- extra/malformed fields on the parsed object are silently dropped rather than propagated. If a future change adds a new field to `AssistantAnswer`, it must be added to `normalizeAssistantAnswer()` too, or it will be silently stripped from every response.
